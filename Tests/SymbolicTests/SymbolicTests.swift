...
class SymbolicTests: XCTestCase {
  func testCurrentLibrary() {
    ...
    XCTAssert(currentObj.path.path.contains("Symbolic"))
    XCTAssertNotNil(currentObj.symbolInfo.path)
    if let file = currentObj.symbolInfo.path {
      XCTAssertEqual(currentObj.path, file)
    }
  ...
  func testLoadLibrary() {
      do {
        let obj = try SharedObject(path: testLibrary("foo"))
